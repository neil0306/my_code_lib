...
#include <vector>
using namespace std;
/// 核心思想：处理过程的目标是【找到左右互为相反数的两个元素，以实现剔除重复数字】
int diffSquareNum(const vector<int>& nums) {
  ...
  int ans = 0;
  while (l <= r) {
    if (nums[l] + nums[r] == 0) {  // l和r位置上的元素刚好互为相反数，需要往数组中心方向缩小区间
      ans += 1;   // 相当于记录了 num[l] 或 num[r] 作为【不同数字】
      int temp = nums[l];
      while (l <= r && nums[l] == temp) {  // 缩小左区间
        l++;
      }
      while (l <= r && nums[r] == -temp) {  // 缩小右区间
        r--;
      }
    } else if (nums[l] + nums[r] > 0) {  // -num[l] < num[r]，需要缩小右区间
      ans += 1;  // 相当于记录了 num[r] 作为【不同数字】
      int temp = nums[r];
      while (l <= r && nums[r] == temp) {
        r--;
      }
    } else {  // -num[l] > num[r]
      ans += 1;  // 相当于记录了 num[l] 作为【不同数字】
      int temp = nums[l];
